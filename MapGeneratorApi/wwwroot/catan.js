...
function generate() {
    ...
    if (document.getElementById('all-diff-no').checked)
        mapDef.diff = false;
    else
        mapDef.diff = true;
...
CatanMap.prototype.generate = function() {
	...
	if (this.mapDefinition) {
        ...
        this.hexTiles = [];
        let tiles = this.hexTiles;
        let isBig = this.mapDefinition.isBig;
        let mapSize = isBig ? 'big' : 'small';
        let diff = this.mapDefinition.diff ? 'diff' : '';
        $.get("api/map/generate/" + mapSize + '/' + diff, function (result) {
            for (let i = 0; i < result.length; ++i) {
                let number = result[i].number;
                let type = '';
                switch (result[i].type) {
                    case 0: type = 'desert'; break;
                    case 1: type = 'grain'; break;
                    case 2: type = 'wood'; break;
                    case 3: type = 'wool'; break;
                    case 4: type = 'ore'; break;
                    case 5: type = 'clay'; break;
                    default: console.log('cell type error');
                }

                let coord = getCoords(i, isBig);
                let x = coord[0];
                let y = coord[1];

                let newHexTile = new HexTile();
                newHexTile.setNumber(number);
                newHexTile.setResourceType(type);
                newHexTile.setCoordinate.apply(
					newHexTile,
					[x, y]
                );
                tiles.push(newHexTile);
            }
            catanMap.resize();
            catanMap.draw();
		});
